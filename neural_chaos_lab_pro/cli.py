...
@app.command()
def train(data: str = "data/series.csv", epochs: int = 5, horizon: int = 10, seq_len: int = 50, device: str = "cpu"):
    df = pd.read_csv(data)
    model = train_lstm(df, seq_len=seq_len, horizon=horizon, epochs=epochs, device=device)
    Path("models").mkdir(exist_ok=True)
    torch.save(model.state_dict(), "models/lstm.pt")
    print("models/lstm.pt")

@app.command()
def forecast_cmd(data: str = "data/series.csv", steps: int = 200, seq_len: int = 50, device: str = "cpu"):
    import numpy as np
    import torch
    ...
    out = Path("reports/forecast.csv"); pd.DataFrame(arr, columns=["x","y","z"]).to_csv(out, index=False)
    print(out)
